...
struct strwithlen
{
	...
	inline const CT *find(const CT *needle, const LT nsize /* needle size */, const LT pos = 0) const
        {
                if (!nsize)
                        return p + pos;
	...
	// We could take this further with loop-unrolling (see "High Performance" document)
	// but it is really not worth it afterall
...
template<typename T>
[[gnu::always_inline]] inline bool TrivialLess(const T &a, const T &b)
{
	return TrivialCmp(a, b) < 0;
}

static constexpr const strwithlen32_t UnsetStrwithlen32;
static constexpr const strwithlen16_t UnsetStrwithlen16;
static constexpr const strwithlen8_t UnsetStrwithlen8;
...
#define _S(p) (p), (uint32_t)(sizeof(p) - 1)
